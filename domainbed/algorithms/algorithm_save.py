...
class Prototype(nn.Module):
	...
	def forward(self, batch_classes, d_idx=0):
		mask = (self.labels.unsqueeze(1)==batch_classes.unsqueeze(0)).sum(1) > -10
...
class ERM(Algorithm):
	...
	def __init__(self, input_shape, num_classes, num_domains, hparams):
		...
		self.classifier = Classifier(feature_dim=self.feature_dim, 
									 n_classes=self.n_classes)
		self.env_classifier = Classifier(feature_dim=self.feature_dim+1, 
									 n_classes=self.n_classes)
	...
	def update(self, x, y, **kwargs):
		...
		for d_index in range(self.n_domain_classes): 
			...
			if domain_loss.item() > self.clip_disc or self.update_count < self.warm_up:
